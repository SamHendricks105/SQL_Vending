...
SELECT cart_item_id, cart_id, machine_id, snack_id, quantity
INTO @id, @cart_id, @machine_id, @snack_id, @quantity
FROM cart_item
WHERE cart_item_id = @lastId;

-- Step 3: Verify the result
SET @testResult = IF(@quantity IN (SELECT quantity FROM cart_item), 'Test Passed', 'Test Failed');
